...
```math
Q=W_QX, \quad K = W_KX, \quad V = W_VX \\ \text{Attention}(Q, K, V) =\text{softmax}\bigg(\frac{QK^T}{\sqrt{d}}\bigg)V
```
...
```math
\phi(x)=(1,kx_1,...,kx_n, lx_1^2, lx_1x_2,...,lx_n^2) \\ \phi^T(Q_i) \phi(K_j) = 1 + k^2Q_i^TK_j+l^2(Q_i^TK_j)^2
```
...
```math
\gamma_Q, \gamma_K, \beta_Q, \beta_K \in \mathbb{R}^{d / h}\\ \phi_Q(Q) \phi^T_K(K) = \big((\gamma_QQ + \beta_Q) (\gamma_KK + \beta_K)^T\big)^2
```
...
Ниже приведены реализации двух кернелов, которыми мы будем пользоваться в дальнейшем (если ничего не изменится в ходе проведения экспериментов). Отмечу, что в такой реализации я не нормирую входной вектор, поскольку подразумеваю наличие слоя LayerNorm перед слоем Attention в блоке трансформера, а также потому, что хочу слегка уменьшить нагрузку на $O(d)$.
